...
class _DocScannerViewState extends State<DocScannerView> {
  ...
  String _sourceLanguage = '한국어';

  final Map<String, TextRecognitionScript> _ocrScripts = {
    ...
    '다른 언어': TextRecognitionScript.latin,
  };

  @override
  void initState() {
    super.initState();
    _targetLanguage = widget.selectedLanguage;
    _initScanner();
    _initTts();
  }
  ...
  Future<void> _startScan() async {
    final String? selectedLanguage = await showDialog<String>(
      ...
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: const Text("스캔할 문서의 언어를 선택해 주세요."),
          actions: [
            Wrap(
              spacing: 8,
              children: _ocrScripts.keys.map((lang) {
                return ActionChip(
                  label: Text(lang),
    ...
    try {
      ...
      if (result != null &&
        ...
        _processOcrAndTranslate();
      }
    } catch (e) {
      if (mounted) {
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(SnackBar(content: Text("스캔 중 에러 발생: $e")));
      }
    }
  }
  ...
  Future<void> _processOcrAndTranslate() async {
    ...
    try {
      final script =
          _ocrScripts[_sourceLanguage] ?? TextRecognitionScript.korean;
      final tempRecognizer = TextRecognizer(script: script);

      final inputImage = InputImage.fromFilePath(_result!.images!.first);
      final recognizedText = await tempRecognizer.processImage(inputImage);

      String cleanText = recognizedText.text.replaceAll('\n', ' ').trim();

      await tempRecognizer.close();
          ...
          Padding(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: _translateLanguages.keys.map<DropdownMenuItem<String>>((
                  String value,
                ) {
