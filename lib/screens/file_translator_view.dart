...
class _FileTranslatorViewState extends State<FileTranslatorView> {
  ...
  late String _selectedSourceLang;
  String _targetLang = '영어';

  @override
  void initState() {
    super.initState();
    _selectedSourceLang = widget.selectedLanguage;
    _initTts();
  }
    ...
    try {
      ...
      if (result != null && result.files.single.path != null) {
        ...
        ScaffoldMessenger.of(
          context,
        ).showSnackBar(const SnackBar(content: Text("파일을 성공적으로 불러왔습니다.")));
      }
    } catch (e) {
      ...
      ScaffoldMessenger.of(
        context,
      ).showSnackBar(SnackBar(content: Text("파일을 불러오는 중 오류 발생: $e")));
    }
  }
  ...
  Future<void> _speak(String text, {bool isOriginal = false}) async {
    ...
    if (!isOriginal) {
      if (_targetLang == '영어') {
        langCode = "en-US";
      } else if (_targetLang == '일본어') {
        langCode = "ja-JP";
      } else if (_targetLang == '중국어') {
        langCode = "zh-CN";
      } else if (_targetLang == '한국어') {
        langCode = "ko-KR";
      }
    } else {
      if (_selectedSourceLang == '영어') {
        langCode = "en-US";
      } else if (_selectedSourceLang == '일본어') {
        langCode = "ja-JP";
      } else if (_selectedSourceLang == '중국어') {
        langCode = "zh-CN";
      } else if (_selectedSourceLang == '한국어') {
        langCode = "ko-KR";
      }
    }
          ...
          Padding(
            ...
            child: DropdownButtonHideUnderline(
              child: DropdownButton<String>(
                ...
                items: _sourceLanguages.keys.map<DropdownMenuItem<String>>((
                  String value,
                ) {
